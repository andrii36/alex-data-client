...
const setSearchProduct = (searchProduct) => ({type: 'SET_SEARCH_PRODUCT', searchProduct})
...
export const clearSearchProduct = () => ({type: 'CLEAR_SEARCH_PRODUCT'})
...
export const getSearchProductThunk = (partNumber) => async (dispatch, getState) => {
    try{
        dispatch(setAllProductsLoading(true))
        const product = await api.getSearchProductsByPartNo(partNumber)
        
        // if(!value) {productsList = await api.getProductsList(getState().content.currentPage)}
        // if(value) {productsList = await api.getSearchProducts({config: getState().content.filterConfig, title: value}, getState().content.currentPage)}
        dispatch(setSearchProduct(product.data.data))
    }catch(err){
        dispatch(setMessageAndCode("Server error", 1))
    }
    dispatch(setAllProductsLoading(false))
...
export const purchaseProductThunk = (id, sellAmount) => async (dispatch) => {
    try{
        const {data} = await api.purchaseProduct(id, sellAmount)
