...
import { addProductThunk, clearCurrentProductAC, clearSearchProduct, getProductByIdThunk, getSearchProductThunk, updateProductThunk } from '../../actions/content-actions'
import { setEditItemSuccess } from '../../actions/modal-modes-actions'
import { Navigate, useLocation, useNavigate } from 'react-router'
import useDebounce from '../../hooks/useDebounce'
import { NavLink } from 'react-router-dom'

const categories = ["Select", "Refrigerators", "Freezers", "Microwave", "Oven", "Stove", "Dishwasher", "Washer", "Dryer"]
...
const EditItem = ({ addProduct }) => {
    ...
    const { title, description, price, available, category, itemsSold, image } = currentProduct
    ...
    return (
        <div style={{ padding: "12px" }}>
            {addProduct && <div style={{ width: "50%" }}>
                <InputText
                    label="Part #:" id="partNumberSearch" placeholder="Enter part number"
            ...
            {searchProduct.length !== 0
                && <div>
                    ...
                    {searchProduct.map(({ partNumber, title, category, available, itemsSold, price, image, _id }) => {
                        return (
                            <Row style={{ padding: "10px", border: "black solid 0.5px" }}>
                                <Col><NavLink to={`/item-details/${_id}`}>{partNumber}</NavLink></Col>
            ...
            {searchProduct.length === 0
                && <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px' }}>
                        <h2>{addProduct ? "Add new product" : "Edit item"}</h2>
                        {code === 1
                            &&
                            ...
                            <Col>
                                ...
                                <Row style={{ textAlign: 'left', display: 'flex', justifyContent: 'space-between' }}>
                                    ...
                                    <Col style={{ textAlign: "center" }}>
                                        ...
                                        <DropdownMenu itemsArr={categories} label="Category:" id="category" title="Cheap"
